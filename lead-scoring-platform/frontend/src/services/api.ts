...
export const scoringApi = {
  recalculateScores: (params: {
    leadIds?: string[];
    campaignId?: string;
    allLeads?: boolean;
  }): Promise<ApiResponse<any>> =>
    api.post('/scoring/recalculate', params).then(res => res.data),
  
  getConfig: (): Promise<ApiResponse<any>> =>
    api.get('/scoring/config').then(res => res.data),
  
  updateConfig: (campaignId: string, config: any): Promise<ApiResponse<any>> =>
    api.put(`/scoring/config/${campaignId}`, config).then(res => res.data),
  ...
  getAnalytics: (params?: {
    ...
    api.get('/scoring/analytics', { params }).then(res => res.data),
  
  getScoreHistory: (leadId: string): Promise<ApiResponse<any>> =>
    api.get(`/scoring/history/${leadId}`).then(res => res.data),
};
...
export const integrationsApi = {
  getIntegrations: (): Promise<ApiResponse<IntegrationSettings>> =>
    api.get('/integrations').then(res => res.data),
  
  updateMetaIntegration: (data: {
    appId: string;
    appSecret: string;
    accessToken: string;
  }): Promise<ApiResponse<any>> =>
    api.put('/integrations/meta', data).then(res => res.data),
  
  updateGoogleIntegration: (data: {
    developerToken: string;
    clientId: string;
    clientSecret: string;
    refreshToken: string;
  }): Promise<ApiResponse<any>> =>
    api.put('/integrations/google', data).then(res => res.data),
  
  disconnectMeta: (): Promise<ApiResponse<void>> =>
    api.delete('/integrations/meta').then(res => res.data),
  
  disconnectGoogle: (): Promise<ApiResponse<void>> =>
    api.delete('/integrations/google').then(res => res.data),
  
  syncMeta: (): Promise<ApiResponse<any>> =>
    api.post('/integrations/meta/sync').then(res => res.data),
  
  syncGoogle: (): Promise<ApiResponse<any>> =>
    api.post('/integrations/google/sync').then(res => res.data),
  
  testConnection: (platform: 'meta' | 'google'): Promise<ApiResponse<any>> =>
    api.post(`/integrations/test/${platform}`).then(res => res.data),
